...
export const BentoItemContainer = ({ children, className }: Props) => {
  ...
  return (
    <div className={`${className} rounded-2xl bg-custom-black h-fit`}>
      <span className="relative inline-flex overflow-hidden rounded-2xl p-[1px] w-full h-full">
        ...
        ></span>
        <div className={`w-full rounded-2xl bg-custom-black backdrop-blur-3xl ${className?.includes("table") ? "" : "p-4"}`}>
          {children}
        </div>
      </span>
    </div>
